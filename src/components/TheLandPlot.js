...
const SeedUI = ({seed_data, seedSelect = undefined}) => {
    ...
    return (
        <div className={`seed_ui ${isPlanted ? 'planted' : 'idle'}`} onClick={seedSelect ? () => seedSelect(seed_data) : null}>
            {
                isPlanted ? (
                        <>
...
const MemoryConfirmation = ({ updatePlantingStatus, updateSelectedSeed, isVisible, setMemCollected }) => {
    ...
    let {playCancel, playObjPlaced} = useSound();
    const handleConfirm = () => {
        playObjPlaced();
...
export default function Gardening() {
    ...
    let {playCancel, playPop} = useSound();
