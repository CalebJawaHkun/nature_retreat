...
export default function PopupFeedback({whichActivity, isVisible, updateVisibility}) {
    ...
    let MessagesForActivity = Messages_conve[whichActivity];
    let choosenMessage = MessagesForActivity[Math.floor(Math.random() * MessagesForActivity.length)];
    console.log(choosenMessage);

    const {updateFbVisibility, updateActPartook} = useFeedback();
    const { playClick } = useSound();

    const handleClose = () => {
        playClick();
        updateActPartook(undefined);
        updateFbVisibility(false);
    }
    //console.log('Popup Reached!');
    return(
        <>
            {
                chosenFB && choosenMessage && 
                (
                        ...
                        <div className='feedbackcon' style={{display: isVisible ? 'block' : 'none'}}>  
                            <div className='feedback' style={{
                                ...
                                <button className='closepopup' onClick={handleClose}>
                                    <img src={crossui} alt={'Close Popup'}  title={'Remove Popup'}/>
                                </button>
                                <p> {choosenMessage} </p>
                            </div>
                        </div> 
